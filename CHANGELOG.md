...
- fix(docs): record why the evidence loop cannot close yet - `<SSB_RATINGS_DIR>/<source>-<league>-<season>.json` has no date component, so every refresh overwrites the very snapshot whose predictions a later settled result would be scored against. Snapshots must be retained per `asOf` before any source can be scored from live data.
