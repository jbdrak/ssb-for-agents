...
- fix: **two harness/data bugs that each manufactured a fake betting edge**, both caught by cross-checking against Brier. (1) The ESPN odds fetch took `items[0]`, but item ORDER varies by league and season, and some books quote a 3-way regulation market whose home+away prices sum to ~0.83 with the draw omitted. De-vigging that invents a negative margin: it made NHL show a **+9.84% favourite ROI** on a hold of -6.43%. The collector now selects the first COHERENT 2-way item (sum 1.00-1.12), preferring one provider for cross-league comparability. (2) `team-validate.js` pooled over season PAIRS, putting every test game into the pool four times, so the z-scores and bootstrap treated correlated duplicates as independent -- it reported z=4.75 and "100% of resamples profitable" for a model that is WORSE than the market on Brier in 17 of 20 season pairs. Each game is now scored once, using the mean probability across training fits. The corrected CFB figure is z=1.72 / 61.8% profitable, and MLB is unchanged at 0.0% -- confirming the fix did not simply kill all signal.
- feat: `scripts/market-efficiency.js` -- cross-league market efficiency. Compares returns rather than Brier, because raw Brier is not comparable across leagues (lopsided slates score better without being more efficient). Includes a hard guard rejecting any price pair whose implied sum is below 1.0.
- feat: `scripts/favourite-bias.js` -- tests the classic favourite-longshot bias against the hold, across five thresholds and five leagues, with per-season stability. Result: the bias is real (UFC calibration slope 1.077) but at ~2-3 points it is smaller than the ~4-5% hold. Every league, every threshold: ROI negative, z negative or ~0.
- feat: UFC, NHL and WNBA support in the collector (UFC needed athlete-shaped odds, `order`-based sides, winner-based settlement, and a separate competition id).
...
- feat: **an honest MLB model, and the honest answer: it does not beat the closing line.** The scan could never have had an edge - it de-vigged public prices and compared them to public prices, which is circular by construction. The only path that does not require private information is a model whose inputs are independent of the price it is compared against. So this builds one (log5 on Pythagorean team strength + a starting-pitcher term + home field), validates it across **three full seasons and 7,147 priced games**, and reports what it actually does. **Verdict: NO EDGE.** Pooled out-of-sample across all six season pairs, the model's ROI is **-4.37%** - which _is_ the market's hold, the signature of a strategy with zero information. It is _worse_ than the market on Brier and log loss in all six pairs. Bootstrap: **0.0% of 2,000 resamples profitable.** Full writeup in `docs/research/2026-09-18-mlb-model-verdict.md`.
  ...
  **Three bugs had to be found first, and two of them were manufacturing fake alpha:**
  ...
  1. **End-of-season lookahead (fake +11% ROI).** `strength()` read live team state, but rows held a _reference_ to the game object and the state map is only complete _after_ the loop - so every prediction was scored against October's team strength. Turned a coin flip into a 60.9% win rate. Now snapshotted at prediction time, with a test asserting the season's first game has `homeStrength === null`.
  2. **ESPN serves the pitcher's season-final ERA retroactively (fake +4.8% ROI).** Verified across 705 pitcher-seasons: **705 identical on every start, 0 varying** (Miles Mikolas: 36 starts, every one 4.78). The ablation gave it away - "strength only" scored +0.16pts while "ERA only" scored **+6.62pts and +10.46% ROI**. No model built on starter ERA beats an MLB close by 6.6 points. The collector no longer stores the field at all, and `npm run mlb:leak-check` asserts it stays gone.
  3. **A lost-update race in the collector** silently dropped 2,463 games to 403. In `games = games.concat(await fetchDay(d))`, JS evaluates the `games` reference _before_ the await, so six concurrent workers each concat onto a stale array. Only visible against a warm cache. Fixed by collecting per-worker results and flattening once.

  **The subtle trap, documented because a weaker analysis would have declared victory here:** against the **raw** price the model loses (-1.1 to -1.9pts, z = -2.4 to -2.9), but against the **de-vigged** market it is faintly positive at the widest threshold (+0.97pts, z = 2.04). That looks significant and is not: the edge is _smaller than the vig_, so ROI stays negative at every threshold, and it _shrinks_ as conviction rises where a real edge would grow. `simulateBets` now reports both benchmarks so this cannot be misread again.

  Machinery added, reusable for any future idea: `lib/team-model.js` (pure, 25 tests), `scripts/sport-collect.js` (ESPN open+close moneylines for any sport, cached and idempotent), and `scripts/team-validate.js` (the full battery: data sanity, cross-season holdout, pooled betting against both benchmarks, bootstrap, placebo, ablation). The placebo control is the one that matters: zero-information models with matched spread return ~0 edge and -1% to -6% ROI, proving the selection procedure is not the artifact.

- feat: **the same battery run on college football, and a genuine near-miss.** NCAAF was worth testing because college markets are thinner, and it is the **only** market tested so far where a public-data model shows a de-vigged edge that clears |z| = 2 (+1.45 to +1.89pts, z = 2.25 to 2.55) and sits above the zero-information placebo range (+0.69 to +1.44pts). **It still loses money** — ROI -0.67% to -2.20% at every threshold, because the edge is smaller than the vig, and **0.0% of 2,000 bootstrap resamples are profitable**. Against the raw price it has nothing at all (z ~ 0), the CLV is negative, and it is worse than the market on Brier in **0 of 20** season pairs. Full writeup: `docs/research/2026-09-18-ncaaf-model-verdict.md`.

  **The generalisation this required, verified against the MLB numbers.** `lib/mlb-model.js` became `lib/team-model.js` (two sports now, so the name had to stop lying), and six single-sport scripts collapsed into `scripts/team-validate.js` + `scripts/sport-collect.js`. The safety net was that **MLB had to reproduce its shipped numbers exactly** through the generic path: -4.37% ROI, +0.16pts (z=0.24), -6.39% blind favourites, 0.0% bootstrap, 0 of 6 Brier pairs. It does.

  Two data findings worth keeping: ESPN's odds path needs a `leagues/` segment (omitting it 404s for **every** league and looks like "this sport has no odds"), and **ESPN's CFB odds archive is sparse and biased** — 24% coverage for 2021 rising to 84% for 2024, and the games with no odds average a **31.9-point** margin versus **15.5** for the ones that have them. The archive omits the blowouts.
