...
- feat: **an honest MLB model, and the honest answer: it does not beat the closing line.** The scan could never have had an edge - it de-vigged public prices and compared them to public prices, which is circular by construction. The only path that does not require private information is a model whose inputs are independent of the price it is compared against. So this builds one (log5 on Pythagorean team strength + a starting-pitcher term + home field), validates it across **three full seasons and 7,147 priced games**, and reports what it actually does. **Verdict: NO EDGE.** Pooled out-of-sample across all six season pairs, the model's ROI is **-4.37%** - which _is_ the market's hold, the signature of a strategy with zero information. It is _worse_ than the market on Brier and log loss in all six pairs. Bootstrap: **0.0% of 2,000 resamples profitable.** Full writeup in `docs/research/2026-09-18-mlb-model-verdict.md`.
  ...
  **The subtle trap, documented because a weaker analysis would have declared victory here:** against the **raw** price the model loses (-1.1 to -1.9pts, z = -2.4 to -2.9), but against the **de-vigged** market it is faintly positive at the widest threshold (+0.97pts, z = 2.04). That looks significant and is not: the edge is _smaller than the vig_, so ROI stays negative at every threshold, and it _shrinks_ as conviction rises where a real edge would grow. `simulateBets` now reports both benchmarks so this cannot be misread again.
  ...
  Machinery added, reusable for any future idea: `lib/mlb-model.js` (pure, 25 tests), `scripts/mlb-collect.js` (ESPN open+close moneylines, cached and idempotent), plus `mlb:backtest`, `mlb:cross-season`, `mlb:stress`, `mlb:placebo`, `mlb:leak-check`. The placebo control is the one that matters: zero-information models with matched spread return ~0 edge and -1% to -6% ROI, proving the selection procedure is not the artifact.
