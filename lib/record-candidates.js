...
function normalizeRow(play, block, scanId, capturedAt) {
  ...
  const playId = play.playId ?? null;
  // Scan rows carry `playId` and NOT `gameId` — verified on a live slate, 129 of
  // 129 rows had a playId and 0 had a gameId. Recording only `play.gameId` is
  // why every candidate in the ledger had `gameId: null`, and a candidate with
  // no fixture id can be neither re-priced (capture-close) nor settled.
  const gameId = play.gameId ?? gameIdFromPlayId(playId);
  ...
  candidate.featureSnapshot = buildFeatureSnapshot(play, { capturedAt, block });
  return candidate;
}
 ...
 */
function buildFeatureSnapshot(play, { capturedAt = null, block = null } = {}) {
  ...
  const snapshot = {
    ...
    // The book whose price we would actually take. Read from the row first, then the
    // scan's target book, then the group. Without the fallbacks every candidate recorded
    // `book: null` even though the scan payload carried `targetBook` — which made the
    // sharp-move-lag question ("did we bet a book that had not repriced yet?")
    // unanswerable, because the record never said which book we would have bet.
    book: play.book ?? play.targetBook ?? block?.book ?? null,
    line: play.line ?? null,
    ...
    // Cross-venue arbitrage margin in percentage points, or null. Recorded so the
    // ledger can answer "were any of these rows arbs?" without re-scanning.
    arbMarginPct: value('arbMarginPct'),
    // Explicit-only: never derived from edge/odds/no-vig or any other field.
    marketFairProbability: value('marketFairProbability'),
    // Sharp-anchored fair + contributor count, carried through so the sharp EV can be
    // measured against realised closes. Never derived from the all-books fair.
    sharpMarketFairProbability: value('sharpMarketFairProbability'),
    sharpFairBookCount: value('sharpFairBookCount'),
...
/**
 * The fixture id embedded in a playId.
 *
 * A playId is `<League>:...:<gameId>::<Market>::<selection>`, so everything
 * before the first `::` is the fixture. This is the same derivation
 * `bin/pp-cli.js`'s `cmdValidate` applies before calling `validate_play`, which
 * REQUIRES a gameId — so a candidate that dropped its playId cannot be
 * re-priced at all.
 *
 * @param {unknown} playId
 * @returns {string|null}
 */
function gameIdFromPlayId(playId) {
  if (typeof playId !== 'string' || playId.trim() === '') return null;
  const head = playId.split('::')[0].replace(/:+$/, '').trim();
