...
const { fairProbabilityForRow, sharpFairProbabilityForRow, sharpFairDetail } = require('./screen-fair-probability');
...
/**
 * Schedule sources that mean "corroborated". Everything else is a fallback.
 * Vocabulary is set by the tennis time correction in lib/ssb-tennis.js:
 * `flashscore` / `flashscore-verified` (0.95), `flashscore (time unparsed)`
 * (0.3), `web_search` (0.6), `pp-mcp (unverified)` (0.3).
 */
const VERIFIED_TENNIS_START_SOURCES = new Set(['flashscore', 'flashscore-verified']);

/**
 * Start-time note for a tennis candidate.
 *
 * The correction records where each start time came from, and `correctTennisTimes`
 * already refuses to treat a fallback as trusted internally. Rendering a single
 * fixed "scheduled start" note for every source throws that distinction away, so
 * a web-search estimate (confidence 0.6) reads exactly like a verified time.
 * Surfacing the source is the whole point: PP times are not to be trusted, and a
 * note that never says "unverified" quietly launders them.
 *
 * @param {Object} row
 * @returns {string|null}
 */
function tennisStartNote(row = {}) {
  if (String(row.league || '').toLowerCase() !== 'tennis') return null;
  const source = String(row.startSource || '')
    .trim()
    .toLowerCase();
  if (VERIFIED_TENNIS_START_SOURCES.has(source)) {
    return 'Scheduled start — tennis matches may be delayed';
  }
  if (!source) return 'Start time NOT corroborated by Flashscore — verify before betting';
  const confidence = Number.isFinite(Number(row.startConfidence)) ? Number(row.startConfidence) : null;
  const qualifier = confidence === null ? '' : ` (confidence ${confidence})`;
  return `Start time from ${source}${qualifier}, NOT Flashscore-verified — verify before betting`;
}
...
function mapCandidateRow(row = {}) {
  ...
  return {
    ...
    startNote: tennisStartNote(row),
    // Carry the schedule provenance alongside the note: the note explains it to
    // a human, these let a consumer act on it without parsing prose.
    startSource: row.startSource ?? null,
    startConfidence: row.startConfidence ?? null,
    odds:
    ...
    // The book whose price we would actually take. `row.book` is not always set on the
    // rows that reach here, but the scan's own target book IS carried as `targetBook` on
    // the same row — without the fallback every candidate recorded `book: null`, which
    // made the sharp-move-lag hypothesis UNTESTABLE: you cannot ask "did we bet a book
    // that had not repriced?" when the record never says which book we would bet.
    book: row.book ?? row.targetBook ?? null,
    ...
    // The BEST price available for this selection across the scanned books, and the
    // price actually being taken (`odds`, i.e. the target book's quote). Recorded
    // together so the shopping gap is MEASURABLE: line shopping is the most reliable
    // retail edge there is, and without the number the ledger could only say whether
    // execution was GRADED well (executionQuality), never what price was left on the
    // table. The expander computes this (screen-row-expander.js) and the ranker passes
    // it; it has to be named here or the snapshot records null and the gap is
    // unknowable. Null when no comparison book posted a parseable line — never
    // fabricated, and never silently defaulted to the target price (which would
    // report a zero gap that was never observed).
    bestAvailableOdds:
      row.bestAvailableOdds == null
        ? null
        : Number.isFinite(Number(row.bestAvailableOdds))
          ? Number(row.bestAvailableOdds)
          : null,
    // Cross-venue arbitrage margin for this row's market, in percentage points, or null
    // when it is not arbitrageable. Arbitrage is the one retail edge the literature
    // documents as prediction-free, so it is recorded rather than only looked for on
    // request. Null is the healthy case — a normal market's implied probabilities sum
    // above 1 — so a ledger full of nulls means no arbs, not a broken field.
    //
    // COMPUTED HERE from `allBookOdds`, deliberately, rather than read off the row: a
    // value that only arrives if an upstream layer happened to set it would record null
    // for a genuine arb the moment that layer changed, and null is indistinguishable
    // from "not an arb". The mapper already derives both fair probabilities from the
    // same map, so this is the same pattern.
    arbMarginPct: arbMarginForRow(row),
    // Decision-time de-vigged fair probability for this side, derived from the
    // market's own two-sided book prices (`allBookOdds`, which survives to here
    // on every row shape). Null when the side cannot be resolved or no book
    // quoted both legs, so a vigged single-sided price is never presented as a
    // fair price. Consumed by the ratings evidence gate; it never feeds ranking.
    marketFairProbability: fairProbabilityForRow(row),
    // Sharp-anchored fair, recorded ALONGSIDE the all-books one so the two can be
    // compared head-to-head against realised closes. The all-books number averages
    // square books into the fair and is contaminated; this one is de-vigged from the
    // repo's sharp comparison set only, so its EV measures our price against the
    // SHARP market. Null (never a silent fallback) when no sharp book quotes both
    // legs. Recording both is what makes the comparison possible at all — the raw
    // per-book odds are not persisted, so neither can be recomputed after the fact.
    sharpMarketFairProbability: sharpFairProbabilityForRow(row),
    sharpFairBookCount: (sharpFairDetail(row) || {}).sharpBooks || 0,
...
module.exports = { mapCandidateRow, tennisStartNote };
