...
/**
 * The display form of a price, or null for a missing value.
 *
 * `odds` in a structured payload must stay a PRICE. It is what JSON consumers
 * read — the `--record-scan` path writes it into the ledger, and EV/CLV
 * arithmetic divides by it — so a display string in that field is a schema
 * violation that is invisible until something computes with it. That is exactly
 * what happened: 64 of 90 recorded decision prices read `'49.0%'`. The NoVig
 * probability convention belongs to presentation, so it now travels in
 * `oddsDisplay` instead.
 *
 * @param {number|string|null|undefined} odds
 * @param {Object} [bet={}]
 * @param {Object} [context={}]
 * @returns {string|null} display quote, or null when there is no price
 */
function oddsDisplay(odds, bet = {}, context = {}) {
  if (odds === null || odds === undefined || odds === '') return null;
  const display = oddsValueForDisplay(odds, bet, context);
 ...
 * @param {string} [bet.startSource] - Schedule source the start time came from.
 * @param {number} [bet.startConfidence] - Confidence in that source.
 ...
 * @param {string} [bet.startCST] - Start time in CST.
 * @param {number|string} [bet.start] - Machine-readable event start (epoch
 *   seconds/ms or ISO) from the ranker row. Carried because `startCST` is a
 *   year-less display string and cannot be converted to an epoch.
 * @param {string} [bet.startNote] - Start time note.
 * @param {string} [bet.startSource] - Schedule source the start time came from.
 * @param {number} [bet.startConfidence] - Confidence in that source.
 * @param {string} [bet.selection] - Player/team selection.
...
function formatBetCompact(bet = {}, context = {}) {
  ...
  return {
    ...
    // Machine-readable event start, straight from the ranker row (the backend's
    // own `start`: epoch seconds for MLB/WNBA/NBA/NFL/NHL, ISO for Soccer/
    // Tennis). `startCST` below is a DISPLAY string with no year and cannot be
    // turned into an epoch, so anything that needs a real instant - the
    // external-ratings recency gate, the tools-mode `--record-scan` ledger,
    // start sorting - must read this field, not the display string.
    start: bet.start ?? null,
    startCST: bet.startCST || null,
    startNote: bet.startNote || null,
    // Where the start time came from and how confident that source is. The
    // tennis correction records this (see lib/ssb-tennis.js) and refuses to
    // treat a fallback as trusted, but dropping it here made a web-search
    // estimate render identically to a Flashscore-verified time.
    startSource: bet.startSource ?? null,
    startConfidence: bet.startConfidence ?? null,
    ...
    oddsDisplay: oddsDisplay(bet.odds ?? bet.targetBookOdds ?? null, bet, context),
    targetBookOdds: bet.targetBookOdds ?? bet.odds ?? null,
    bestAvailableOdds: bet.bestAvailableOdds ?? null,
    executionQuality: bet.executionQuality || null,
    // Decision-time de-vigged fair probability for this side. Derived in the
    // candidate mapper from the market's own two-sided prices (see
    // lib/screen-fair-probability.js) and carried through so `--record-scan`
    // writes it into the ledger for the ratings evidence gate. Null when the
    // side or the two-sided prices could not be resolved - never a guess.
    marketFairProbability: /** @type {any} */ (bet).marketFairProbability ?? null,
    // Sharp-anchored fair, recorded ALONGSIDE the all-books one. The all-books fair
    // averages square books into the fair, which turns EV into a contaminated
    // line-shopping test; this one de-vigs the sharp comparison set only, so its EV
    // measures our price against the sharp market. Null (never a fallback to the
    // all-books answer) when no sharp book quotes both legs. Both are written so the
    // two anchors can be compared head-to-head against realised closes.
    sharpMarketFairProbability: /** @type {any} */ (bet).sharpMarketFairProbability ?? null,
    sharpFairBookCount: /** @type {any} */ (bet).sharpFairBookCount ?? null,
    ...
    // The honest name for the same number: the move from the opening price to
    // the current one. It is NOT closing line value — that is `clvPct`, which
    // needs a captured close (scripts/capture-close.js). Read through a loose
    // cast: these are new optional fields not yet on the bet typedef.
    openToCurrentPct: /** @type {any} */ (bet).openToCurrentPct ?? bet.clvProxyPct ?? bet.clv ?? null,
    closeOdds: /** @type {any} */ (bet).closeOdds ?? null,
    clvPct: /** @type {any} */ (bet).clvPct ?? null,
...
const STANDARD_KEEP_FIELDS = new Set([
  ...
  'targetBookOdds',
  'bestAvailableOdds',
  // Cross-venue arbitrage margin, percentage points, or null when not arbitrageable.
  'arbMarginPct',
  'executionQuality',
  // Decision-time de-vigged fair probability for this side. Recorded so the
  // ratings evidence gate can compare a model's probability against the price
  // actually available. Ranking never reads it.
  'marketFairProbability',
  // Sharp-anchored fair and how many sharp books contributed to it. Recorded next to
  // the all-books fair so the two anchors can be compared against realised closes.
  // Ranking never reads either.
  ...
  'market',
  'targetBook',
  // The book whose price we would take. Kept explicitly because the sharp-move-lag
  // hypothesis ("did we bet a book that had not repriced yet?") is unanswerable without
  // it — the record must say which book we would have bet, not just which books quoted.
  'book',
  'movementDisposition',
  'movementSourceBook',
  'movementMode',
...
function formatBetStandard(bet = {}, context = {}) {
  ...
  // Prices stay prices in the structured payload; the display form is separate
  // and never overwrites the value a consumer computes with. `oddsDisplay`
  // returns null for a missing value, so no extra emptiness guard is needed.
  if (out.odds !== undefined && out.odds !== null) {
    out.oddsDisplay = oddsDisplay(out.odds, bet, context);
  }
  return out;
}
...
function formatQuickScreenMinimal(response = {}) {
  ...
  out.plays = allPlays.map((p) => ({
    ...
    odds: p.odds ?? p.targetBookOdds ?? null,
    oddsDisplay: oddsDisplay(p.odds ?? p.targetBookOdds ?? null, p, context),
    targetBookOdds: p.targetBookOdds ?? p.odds ?? null,
    bestAvailableOdds: p.bestAvailableOdds ?? null,
    executionQuality: p.executionQuality || null,
    // Decision-time de-vigged fair probability for this side, carried from the
    // expander's two-sided de-vig. Null when no comparison book quoted both
    // legs of the market. Recorded for the ratings evidence gate; it never
    // feeds ranking or the verdict.
    marketFairProbability: p.marketFairProbability ?? null,
    // Sharp-anchored counterpart of the line above: the same de-vig but over the
    // sharp comparison set only. Null when no sharp book quoted both legs.
    sharpMarketFairProbability: p.sharpMarketFairProbability ?? null,
    sharpFairBookCount: p.sharpFairBookCount ?? null,
