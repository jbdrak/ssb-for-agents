 ...
 * @returns {{outcomes: Array<{league: string, game: string, winner: string}>, matched: number, unmatched: number, refused: number, reasons: Object}}
 */
function matchCfbOutcomes(records, index) {
  ...
  for (const record of Array.isArray(records) ? records : []) {
    const league = record && record.league;
    const key = record ? fixtureKey(record.teamA, record.teamB, league) : null;
    if (!key) {
  ...
  return { outcomes, matched: outcomes.length, unmatched, refused: unmatched, reasons };
}

module.exports = { buildCfbOutcomeIndex, matchCfbOutcomes, fixtureKey };
