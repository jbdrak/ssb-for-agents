...
/**
 * Probable starter for one competitor, or null when ESPN has not posted one.
 *
 * WHY THIS IS PARSED HERE. The scoreboard payload already carries `probables` on each
 * competitor, so exposing them costs no extra request. Without this, anything needing a
 * probable starter (promo-card matchup passes, game previews) has to bypass the resolver
 * and hand-roll its own ESPN fetch — which is exactly what happened, and it meant the
 * repo had two ESPN call paths instead of one.
 *
 * Returns null rather than a placeholder when ESPN has not announced a starter: "not
 * posted" and "a pitcher whose name is unknown" are different facts, and a matchup read
 * that cannot tell them apart is worse than one that admits it does not know.
 *
 * @param {Object} competitor - an ESPN competitor object
 * @returns {{name: string, era: string|null, wins: string|null, losses: string|null}|null}
 */
function probableStarterOf(competitor) {
  const probable = Array.isArray(competitor?.probables) ? competitor.probables[0] : null;
  const name = probable?.athlete?.displayName;
  if (!name) return null;
  const stats = {};
  for (const entry of Array.isArray(probable.statistics) ? probable.statistics : []) {
    if (entry && entry.name != null) stats[entry.name] = entry.displayValue ?? null;
  }
  return {
    name: String(name),
    era: stats.ERA ?? null,
        ...
        for (const event of events) {
          ...
          for (const comp of competitions) {
            ...
            allCompetitions.push({
              ...
              winner,
              date: compDate,
              // Additive: existing consumers read named keys, so new fields cannot break
              // them. Both come from the same payload, so this costs no extra request.
              venue: comp?.venue?.fullName || null,
              homeProbable: probableStarterOf(home),
              awayProbable: probableStarterOf(away)
            });
          }
        }
      } catch {
