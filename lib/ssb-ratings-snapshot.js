...
// The source segment allows an underscore because the contract's own source ids
// include them (`tennis_elo`, `massey_games`). A source name that cannot match
// here is not merely unlisted - its snapshot is invisible to every read path,
// which is how a source can look like it has no data at all.
const SNAPSHOT_FILE_PATTERN = /^([a-z_]+)-([A-Za-z0-9]+)-(\d{4})\.json$/;
...
const HISTORY_FILE_PATTERN = /^([a-z_]+)-([A-Za-z0-9]+)-(\d{4})-(\d{4}-\d{2}-\d{2})\.json$/;
