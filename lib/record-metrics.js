...
/**
 * Statuses on a bet/settlement that mean the outcome is DECIDED. A `pending`
 * (or otherwise non-decided) settlement must never be treated as an outcome.
 */
const DECIDED_OUTCOMES = ['win', 'loss', 'push'];
/** 95% two-sided. */
const Z_95 = 1.96;
...
function beatTheCloseReport(ledger, opts = {}) {
  ...
  // A candidate can only ever receive a close if its fixture is resolvable, because
  // `validate_play` requires a gameId. Rows recorded before playId was persisted have
  // neither a gameId nor a playId, so they are permanently unmeasurable — and the scan
  // records do NOT keep the raw plays, so they cannot be repaired from the ledger.
  // Counting them inside "no close" makes a structurally impossible close look like a
  // failing sweep, so they are split out.
  let notYetClosed = 0;
  let neverClosable = 0;
  ...
  for (const candidate of candidates) {
    ...
    // The same play can sit in the ledger more than once: record-scan is keyed on the
    // scan record's content, so recording one scan file twice produces two scan ids and
    // therefore two candidate ids for one observation. Dedupe FIRST, before the
    // close-eligibility split — otherwise the closable bucket is inflated by
    // bookkeeping (measured on the live ledger: 209 counted vs 158 actually distinct,
    // a 24% overstatement of the rows that can still be measured).
    const key = [
    ...
    if (!hasClose) {
      withoutClose += 1;
      if (candidate.gameId || candidate.playId) notYetClosed += 1;
      else neverClosable += 1;
      continue;
    }
...
function settledRows(ledger) {
  ...
  for (const bet of bets) {
    ...
    // A settlement only wins when it is DECIDED. `settle-record` writes a
    // `pending` row for every bet it cannot match -- including migrated bets that
    // carry their real result on the bet row with an unresolvable start
    // (`eventDate: 'unknown'`). Treating that pending row as the outcome SHADOWS
    // the bet's own decided status and silently collapses a real record to 0W/0L.
    // Documented precedence: a decided settlement, else the bet's own outcome.
    const settlementOutcome = settlement ? settlement.status : null;
    const outcome = DECIDED_OUTCOMES.includes(settlementOutcome) ? settlementOutcome : bet.status;
    if (!DECIDED_OUTCOMES.includes(outcome)) continue;
