...
function beatTheCloseReport(ledger, opts = {}) {
  ...
  for (const candidate of candidates) {
    ...
    // A close can only come from the row that was recorded with a resolvable
    // fixture, and the same play can sit in the ledger more than once: record-scan
    // is keyed on the scan record's content, so recording one scan file twice
    // produces two scan ids and therefore two candidate ids for one observation.
    // Count each (fixture, market, selection, price) once, or the beat rate is
    // computed over a denominator inflated by bookkeeping.
    const key = [
