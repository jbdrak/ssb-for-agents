...
function createFallbackPlay({
  ...
  return {
    ...
    movementDisposition,
    // Mirror the disposition into `movement`, exactly like every other play
    // row. The compact scan/rank output renames movementDisposition -> movement
    // for normal rows; without this the fallback rows ship `movement: null`
    // while movementDisposition holds the real value, so any consumer reading
    // the documented `movement` field sees null and mis-classifies the row.
    movement: movementDisposition,
    edge: roundedClv,
...
async function collectTennisFallbackCandidates({ client, book, markets, cardWindow, skipTimeCorrection = false }) {
  ...
  for (const market of markets) {
    ...
    // Correct start times BEFORE the date window. PP's raw gameId timestamp is
    // stale (often a day off) and is only corrected later, on the surviving
    // plays - so a window applied to the raw value filters the wrong day and
    // serves next-day matches from a `today` scan. Cache + ESPN only here; the
    // full correction (with the per-match web resolver) still runs on the
    // plays built below.
    if (!skipTimeCorrection && games.length) {
      try {
        await correctTennisTimes(games, { skipUnmatched: true });
      } catch {
        // Best effort: the window falls back to the raw times.
      }
    }
