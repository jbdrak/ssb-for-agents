'use strict';
/**
 * A real, honest team-strength model for any sport. Sport-agnostic: MLB and college
 * football both run through this file, differing only in the Pythagorean exponent, the
 * minimum-games threshold, and whether the sport has a starting pitcher.
 *
 * The point of this module is not to be clever. It is to be a model whose inputs are
 * INDEPENDENT of the price it is compared against: team scoring margin (Pythagorean
 * expectation) and, where the sport has one, the starting pitcher. Everything the scan ever
 * did was a de-vig of public prices compared to public prices, which cannot produce an edge
 * by construction.
 *
 ...
 *      `buildWalkForwardRows` snapshots team state BEFORE recording the game's own result.
 *   2. COEFFICIENTS ARE FIT ON TRAIN AND EVALUATED ON HELD-OUT DATA. Never fit and
 *      report on the same games. Prefer a different SEASON over an in-season split.
 *   3. Report UNCERTAINTY. A 3-point edge on 500 bets is noise; say so.
 *   4. A model with no information returns roughly minus the hold. If a result is much
 *      better than that, suspect a leak before believing it.
 */

const PYTHAGOREAN_EXPONENT = 1.83; // baseball; football wants ~2.37
 ...
 */
function buildWalkForwardRows(games, { minGames = 10, minStarts = 3, exponent = PYTHAGOREAN_EXPONENT } = {}) {
  ...
  const snapshot = (id) => {
    ...
    if (!t) return null;
    return pythagoreanStrength(t.rs, t.ra, t.games, { minGames, exponent });
  };
