...
//      (`PP_RATINGS_DIR` → `~/.ssb-for-agents`, never the repo), matching
//      lib/ssb-ratings-snapshot.js; and
 ...
 * override convention: `PP_RATINGS_DIR`, else `~/.ssb-for-agents`.
 */
function stateDir() {
  const envDir = process.env.PP_RATINGS_DIR;
  if (typeof envDir === 'string' && envDir.trim() !== '') return envDir;
  return path.join(os.homedir(), '.ssb-for-agents');
}

/**
 * Default snapshot path: `$PP_TENNIS_ELO_SNAPSHOT`, else
 * `<stateDir>/tennis-elo-snapshot.json` — always outside the repo.
 */
