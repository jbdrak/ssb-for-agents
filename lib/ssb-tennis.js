...
async function correctTennisTimes(rows, { skipUnmatched = false } = {}) {
  ...
  // The web/ESPN-search resolver is expensive (per-match scraping), so callers
  // that only need a usable date (e.g. a pre-window date filter that runs
  // again later on the survivors) pass skipUnmatched. The cache and ESPN passes
  // above still run.
  if (!skipUnmatched) {
