...
//
// Massey states its coverage with ONE phrase in two states: in season the title
// reads `... Using games thru Sun, Sep 13, 2026`, and out of season it reads
// `... Using games thru Preseason`. The second form carries no date at all, so
// `asOf` and `season` cannot be derived and the snapshot store correctly refuses
// the write. That refusal must not be what a reader sees: the page parsed fine
// and the ratings are simply not usable (undated, so unpublishable and never
// safe to read as current), so it is reported as the seasonal condition it is -
// `coverage: 'unavailable'` with a reason quoting Massey's own heading - and no
// records are emitted for a table that can never be persisted. See
// `parseHeading`.
//
...
//
// Massey publishes **no win probability**, so `modelWinProbability` stays null
// on every record. The export does carry `EW`/`EL` (expected wins / losses),
// but those are a season-length expectation for a team, not a per-game
// probability, and no documented rating-to-probability conversion exists for
// Massey. It is therefore scoped OUT of probability scoring rather than scored
// on an invented mapping; the reason string lives in
// `lib/ssb-ratings-evaluation-bridge.js` (`probabilitySupport('massey')`) and is
// what an evaluation reports instead of a score.
//
// This ratings table is not the only Massey page the layer reads. Massey's
// per-sport GAMES board does carry a per-fixture win probability, and
// `./massey-games` reads it as the separate source `massey_games`. The split is
// deliberate: this file's records are team-scoped (no fixture, no probability)
// and that file's are fixture-scoped (with one), and the layer never blends two
// models. "Massey has no win probability" is true of THIS table only.
//
...
// The export's own coverage statement, split into the literal fragment after
// "Using games thru" and, when that fragment is a real date, its parsed parts.
// `([^\r\n]+)` keeps the fragment on one line so `Preseason` and
// `Sun, Sep 13, 2026` are both captured whole.
const ASOF_FRAGMENT_PATTERN = /using games (?:thru|through)\s+([^\r\n]+)/i;
const ASOF_DATE_PATTERN = /(?:[A-Za-z]{3,9},\s*)?([A-Za-z]{3,9})\s+(\d{1,2}),?\s+(\d{4})/;
...
/**
 * Parse a "Using games thru" fragment into `{ asOf, season }`, or `null` when it
 * is not a date. Massey's out-of-season fragment is the literal word
 * `Preseason`, which is why "no match" is a real, expected state rather than a
 * parse failure.
 */
function parseAsOfFragment(fragment) {
  const match = ASOF_DATE_PATTERN.exec(String(fragment || ''));
...
/**
 * The export's title line, as the page itself states it:
 * `fragment` is the literal text after "Using games thru" (`null` when the
 * export carries no such line), and `asOf`/`season` are set only when that
 * fragment is a real date.
 *
 * Keeping the fragment means a caller can quote Massey's own words
 * (`Using games thru Preseason`) instead of inventing a cause for a table that
 * has no date, and letting `asOf` stay `null` is what distinguishes "parsed, but
 * there are no games yet" from "the heading drifted".
 *
 * @param {string} raw
 * @returns {{ fragment: string | null, asOf: string | null, season: number | null }}
 */
function parseHeading(raw) {
  const match = ASOF_FRAGMENT_PATTERN.exec(String(raw || ''));
  if (!match) return { fragment: null, asOf: null, season: null };
  const fragment = match[1].trim();
  const parsed = parseAsOfFragment(fragment);
  return { fragment, asOf: parsed ? parsed.asOf : null, season: parsed ? parsed.season : null };
}
...
function buildRecord(context) {
  ...
  return {
    ...
    marketOpen: null,
    marketCurrent: null,
    // Massey publishes no win probability (see the module header).
...
function normalizeMassey(options) {
  ...
  if (heading.asOf === null) {
    // The table parsed but the export states no coverage date. Massey prints
    // `Using games thru Preseason` out of season, and an undated ratings table is
    // not usable here: the snapshot store requires `asOf`/`season` (so it could
    // never be persisted) and a consumer must never read it as current. Report
    // the seasonal condition in Massey's own words - the snapshot-write refusal
    // it causes is a consequence, and must not be the visible cause.
    return unavailable({
      league,
      reason:
        heading.fragment === null
          ? 'massey export carries no "using games thru" as-of heading'
          : `massey reports no games played yet (heading: 'Using games thru ${heading.fragment}')`,
      sourceUrl,
      sourceHash,
      fetchedAt
    });
  ...
  return {
    ...
    sourceUrl,
    sourceHash,
    // Fail closed: zero records is never `full`. An empty table must not read as
    // success, and `full` must never sit beside `records: 0`.
