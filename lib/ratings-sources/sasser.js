...
//      probability, so none is derived from the projected scores, and
//      `modelWinProbability` stays null. It is therefore scoped out of
//      probability scoring by decision; the reason an evaluation reports lives
//      in `lib/ssb-ratings-evaluation-bridge.js`
//      (`probabilitySupport('sasser')`).
//
// The "no win probability" half of (3) is VERIFIED against the whole page, not
// just the fields this adapter happens to map: the reconstructed `week` object
// carries only `season`, `week`, `label`, `dateRange`, `updatedAt`, `intro`,
// `modelRecord` and `games`, and a per-game object carries only `id`, `day`,
// `kickoff`, `network`, `venue`, `neutralSite`, `away`, `home`, `projection`,
// `market` and `picks`. There is no probability, confidence, edge or winProb
// field anywhere in the payload. The one model-accuracy datum the page does
// publish is `week.modelRecord` (its own season record: straight-up and
// against-the-spread W/L), which is a page-level record rather than a per-game
// probability - it cannot be scored as one and it is not a contract field, so it
// is deliberately not carried. Do not re-open this expecting a hidden
// probability: giving Sasser one would mean inventing a conversion, which is the
// thing this layer refuses.
//
...
function buildRecord(context) {
  ...
  return {
    ...
    // Sasser publishes no win probability (see the module header).
