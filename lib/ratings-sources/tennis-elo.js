...
const { DEFAULT_MIN_SURFACE_MATCHES, DEFAULT_SURFACE_WEIGHT, normalizeSurface } = require('../tennis-elo');
...
function lookupMatch(options) {
  ...
  if (!(snapshotAsOf < predictionDate)) {
    return unavailable(
  ...
  if (isNonEmptyString(snapshotNotBefore)) {
    ...
    if (isDateString(floor) && snapshotAsOf < floor) {
      return unavailable(
        'snapshot_stale',
